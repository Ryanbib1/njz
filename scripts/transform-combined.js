...
function getMapNestingLevel(path) {
  ...
  while (cur) {
    if (cur.isArrowFunctionExpression() || cur.isFunctionExpression()) {
      ...
      if (ce?.isCallExpression() && t.isMemberExpression(ce.node.callee) && t.isIdentifier(ce.node.callee.property, { name: 'map' })) level++;
    }
    cur = cur.parentPath;
  }
  return level;
...
function getArrayName(node) {
  ...
  if (t.isMemberExpression(node)) {
    ...
    return o ? `${o}.${node.property.name || node.property.value}` : null;
  }
  if (t.isCallExpression(node) && t.isMemberExpression(node.callee)) return getArrayName(node.callee.object);
  return null;
}
...
function getMERoot(node) {
  if (t.isIdentifier(node)) return node.name;
  if (t.isMemberExpression(node)) return getMERoot(node.object);
  return null;
}
...
function getMEPath(node) {
  ...
  if (t.isMemberExpression(node)) {
    const p = getMEPath(node.object), prop = node.property.name || node.property.value;
...
function collectParentCtx(path, mapContexts) {
  ...
  while (cur) {
    if (cur.isArrowFunctionExpression() || cur.isFunctionExpression()) {
      const ce = cur.parentPath;
      if (ce?.isCallExpression() && t.isMemberExpression(ce.node.callee) && t.isIdentifier(ce.node.callee.property, { name: 'map' })) {
...
function processCode(source, filePath) {
  ...
  traverse(ast, {
    CallExpression(path) {
      ...
      if (!t.isMemberExpression(callee) || !t.isIdentifier(callee.property, { name: 'map' })) return;
  ...
  traverse(ast, {
    ...
    VariableDeclarator(path) {
      ...
      if (t.isIdentifier(id) && t.isMemberExpression(init) && getMERoot(init) === ctx.itemParam) {
    ...
    JSXExpressionContainer(path) {
      ...
      if (hasAttr(el, 'data-api-bind-info')) return;
      if (t.isMemberExpression(expr) && getMERoot(expr) === ctx.itemParam) {
