...
export interface GoogleLoginUrlOutput {
  url: string
}
...
export const getGoogleLoginUrl = withResult(async (): Promise<GoogleLoginUrlOutput> => {
...
export const handleGoogleCallback = withResult(async (authToken: string): Promise<AdminLoginOutput> => {
  if (!authToken) {
    throw new Error('Google authentication failed.')
  }

  const provider = getAuthProvider('google')
  const googleUser = await provider.getAuthUser(authToken)
