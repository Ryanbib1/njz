...
(function () {
   ...
   * Supports native checkbox and Radix/shadcn Checkbox (button[role="checkbox"]).
   */
  function checkAgreement() {
    ...
    // Native checkbox: use React setter + events
    if (el.type === 'checkbox') {
      var proto = Object.getPrototypeOf(el);
      var desc = proto && Object.getOwnPropertyDescriptor(proto, 'checked');
      if (desc && desc.set) {
        try { desc.set.call(el, true); } catch (e) { el.checked = true; }
      } else {
        el.checked = true;
      }
      el.dispatchEvent(new Event('change', { bubbles: true }));
    ...
    while (cur && depth < 20) {
      var props = cur.memoizedProps || cur.pendingProps;
      if (props && typeof props.onCheckedChange === 'function') {
